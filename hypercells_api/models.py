...
class Context(models.Model):
    ...
    def get_fields(self):
        ...
        if len(self.displayed_fields) > 0:
            return [field for field in fields if (field.name in self.displayed_fields) and field.name != "id"]
        if len(self.hidden_fields) > 0:
            return [field for field in fields if (field.name not in self.hidden_fields) and field.name != "id"]
